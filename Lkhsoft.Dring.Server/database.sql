...
CREATE TABLE IF NOT EXISTS Users (
                                     Username TEXT PRIMARY KEY NOT NULL DEFAULT 'guest', 
                                     Password TEXT NOT NULL DEFAULT '',
                                     Role TEXT NOT NULL DEFAULT 'Guest', 
                                     IsConnected INTEGER NOT NULL DEFAULT 0,
                                     IV TEXT NOT NULL DEFAULT 'none'
);
...
CREATE TRIGGER IF NOT EXISTS UpdateUserOnSessionEnd
    AFTER
        UPDATE
            OF EndTime ON Sessions FOR EACH ROW WHEN NEW.EndTime IS NOT NULL BEGIN
    UPDATE
        Users
    SET
        IsConnected = 0
...
CREATE TRIGGER IF NOT EXISTS UpdateUserOnSessionStart
    AFTER
        INSERT ON Sessions FOR EACH ROW BEGIN
    UPDATE
        Users
    SET
        IsConnected = 1
